...
def prepare_corpus(corpus):
    ...
    dictionary.filter_extremes(
        no_below=5, no_above=0.5, keep_n=2000)  # shape:(18846,24759)
...
def visualizeWrd2V(model):
    ...
    ax = Axes3D(fig)
    plt.scatter(vectors[:, 0], vectors[:, 1], vectors[:, 2],
                c=colors, alpha=1, label=labels)
...
def TSNEVisualize(new_corpus, transformer_type, gnd, semantic_labels):
    ...
    for i in range(k):
        ax.scatter(tsne_X[gnd == i, 0], tsne_X[gnd == i, 1],
                   alpha=.8, label=semantic_labels[i])
    ax.legend(fancybox=True, framealpha=0.5)
    print('>>>>>>>>>>>>>> TSNE Visualization saved <<<<<<<<<<<<')
    fig.savefig(f"./workdir/{transformer_type}_TSNE.png", dpi=fig.dpi)


def NavieBayes(X_train, X_test, Y_train):
    model_nb =MultinomialNB(alpha=.5)
    model_nb.fit(X_train, Y_train)
    return model_nb.predict(X_test)


def bonus():
    data_train = fetch_20newsgroups(subset='train', shuffle=False,
                                    remove=('headers', 'footers', 'quotes'))
    data_test = fetch_20newsgroups(subset='test', shuffle=False,
                                   remove=('headers', 'footers', 'quotes'))
    ...
    Y_predict = NavieBayes(X_train, X_test, Y_train)
    print('NMI score: ', nmi_score(Y_test, Y_predict))
...
def backbone(transformer_type, corpus, dictionary, bow_corpus, model_path=''):
    ...
    if transformer_type == 'TF_IDF':
        ...
        new_corpus = np.transpose(corpus2dense(
            new_corpus, num_terms, num_docs))
        return new_corpus
        ...
        if model_path != '':
            ...
            model = models.Word2Vec(
                corpus, size=vector_size, min_count=min_count, iter=epochs)
